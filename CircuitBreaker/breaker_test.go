...
func succeed(cb *CircuitBreaker) error {
	_, err := cb.Execute(func() (interface{}, bool, error) { return nil, false, nil })
	return err
}
...
func succeedLater(cb *CircuitBreaker, delay time.Duration) <-chan error {
	ch := make(chan error)
	go func() {
		_, err := cb.Execute(func() (interface{}, bool, error) {
			time.Sleep(delay)
...
func fail(cb *CircuitBreaker) error {
	msg := "fail"
	_, err := cb.Execute(func() (interface{}, bool, error) { return nil, false, fmt.Errorf(msg) })
	if err.Error() == msg {
		return nil
	}
	return err
...
func causePanic(cb *CircuitBreaker) error {
	_, err := cb.Execute(func() (interface{}, bool, error) { panic("oops"); return nil, false, nil })
	return err
}
...
func TestCustomIsSuccessful(t *testing.T) {
	isSuccessful := func(bool, error) bool {
		return true
	}
	...
	cb.isSuccessful = func(ignore bool, err error) bool {
		return err == nil
	}
	for i := 0; i < 6; i++ {
		assert.Nil(t, fail(cb))
...
func TestTimeBefore(t *testing.T) {
	t1 := "2022-10-10"
	t2 := "2022-11-11"
	time1, _ := time.Parse(Utils.SimpleLayoutYMD, t1)
	time2, _ := time.Parse(Utils.SimpleLayoutYMD, t2)
